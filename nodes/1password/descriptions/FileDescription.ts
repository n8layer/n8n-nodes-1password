		...
		options: [
			{
				...
				routing: {
					request: {
						method: 'GET',
						url: '/v1/vaults/{{ $parameter.vaultUUID }}/items/{{ $parameter.itemUUID }}/files/{{ $parameter.fileUUID }}/content',
						qs: {
			...
			},
			{
				...
				routing: {
					request: {
						method: 'GET',
						url: '/v1/vaults/{{ $parameter.vaultUUID }}/items/{{ $parameter.itemUUID }}/files/{{ $parameter.fileUUID }}',
						qs: {
							inline_content: '{{ $parameter.inlineContent }}',
						},
					},
...
export const fileFields: INodeProperties[] = [
	{
		...
		displayOptions: {
			show: {
				operation: ['getFiles', 'getFileDetails'],
			},
		},
		default: '',
	},
